...
def test_task_creation(
    client, authenticated_user, second_test_user, test_status
):
...
def test_task_creation_no_login(client, second_test_user):
    ...
    creation_url = urls.reverse("create_task")
    resp = client.post(
        creation_url, {"name": "first", "author": second_test_user.id}
    )
    assert Task.objects.count() == 0
    assert resp.status_code == 302
...
@pytest.mark.django_db
def test_task_delete_by_author(
    ...
    delete_url = urls.reverse(
        "delete_task", kwargs={"pk": test_task_by_auth_user.id}
    )
